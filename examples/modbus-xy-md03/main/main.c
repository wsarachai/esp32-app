 ...
 * 1 = ESP32 + MAX485 + XY-MD03 (Modbus polling)
 * 0 = ESP32 + MAX485 only (transceiver/bus probe)
 */
#define APP_WITH_XY_MD03 1

// Physical wiring for RS485 transceiver:
//
// ESP32               MAX485 Module          XY-MD03
//                    ┌─────────────┐
// GND   ------------>│ GND         │
//                    │ A           │<---------> A+
//                    │ B           │<---------> B-
// 3.3V  ------------>│ VCC         │
// GPIO17 (TX) ------>│ DI          │         VCC ----> 5~24V PSU
// GPIO4  ----------->│ DE          │         GND ----> Common GND
// GPIO4  ----------->│ RE          │
// GPIO16 (RX) <------│ RO          │
//                    └─────────────┘
//                        [120Ω]
//                     A+-----B-  (termination resistor at XY-MD03 end)
...
/*
 * Optional diagnostic scan mode for unknown slave ID.
 * 0 = use MB_SLAVE_ADDR only
 * 1 = scan IDs from MB_SCAN_MIN_ADDR..MB_SCAN_MAX_ADDR each cycle
 */
#define MB_SCAN_SLAVE_ID 1
#define MB_SCAN_MIN_ADDR 1
#define MB_SCAN_MAX_ADDR 10

#define RS485_TEST_PERIOD_MS 1000
#define RS485_TEST_RX_TIMEOUT_MS 100
                                               ...
                                               uint16_t *out_regs)
{
  ...
  uart_wait_tx_done(MB_UART_PORT, pdMS_TO_TICKS(100));
...
static void rs485_only_probe_loop(void)
{
  // Modbus-like probe frame; useful when testing against a USB-RS485 tool.
  uint8_t tx_probe[] = {0x01, 0x03, 0x00, 0x01, 0x00, 0x02, 0x95, 0xCB};
  uint8_t rx_buf[32] = {0};

  while (1)
  {
    uart_flush_input(MB_UART_PORT);
    uart_write_bytes(MB_UART_PORT, (const char *)tx_probe, sizeof(tx_probe));
    uart_wait_tx_done(MB_UART_PORT, pdMS_TO_TICKS(100));

    int rx_len = uart_read_bytes(MB_UART_PORT,
                                 rx_buf,
                                 sizeof(rx_buf),
                                 pdMS_TO_TICKS(RS485_TEST_RX_TIMEOUT_MS));

    if (rx_len > 0)
    {
      ESP_LOGI(TAG, "RS485-only probe: received %d byte(s)", rx_len);
    }
    else
    {
      ESP_LOGI(TAG, "RS485-only probe: no data (expected with no remote node)");
    }
...
void app_main(void)
{
...
#if APP_WITH_XY_MD03
  ESP_LOGI(TAG, "Mode: ESP32 + MAX485 + XY-MD03");

#if MB_SCAN_SLAVE_ID
  ESP_LOGI(TAG, "Slave scan enabled: addr %d..%d", MB_SCAN_MIN_ADDR, MB_SCAN_MAX_ADDR);
#endif
  ...
  while (1)
  {
    ...
    uint8_t active_addr = MB_SLAVE_ADDR;

#if MB_SCAN_SLAVE_ID
    for (uint8_t addr = MB_SCAN_MIN_ADDR; addr <= MB_SCAN_MAX_ADDR; addr++)
    {
      err = modbus_read_holding_registers(addr,
                                          MB_REG_TEMPERATURE,
                                          2,
                                          regs);
      if (err == ESP_OK)
...
#else
    err = modbus_read_holding_registers(MB_SLAVE_ADDR,
                                        MB_REG_TEMPERATURE,
                                        2,
                                        regs);
#endif
    ...
    if (err == ESP_OK)
    {
      ...
      ESP_LOGI(TAG,
       "Slave:%u Temperature: %.1f C, Humidity: %.1f %%RH",
       active_addr,
               (double)temp_c,
               (double)hum_rh);
    }
...
#if MB_SCAN_SLAVE_ID
  ESP_LOGW(TAG,
       "Read failed on all slave IDs (%d..%d): %s",
       MB_SCAN_MIN_ADDR,
       MB_SCAN_MAX_ADDR,
       esp_err_to_name(err));
#else
...
#else
  ESP_LOGI(TAG, "Mode: ESP32 + MAX485 only");
  rs485_only_probe_loop();
#endif
}