...
/**
 * AP SSID handler - returns the configured AP SSID
 * GET /apSSID.json
 */
...
/**
 * WiFi connect handler - updates STA configuration and starts connection.
...
/**
 * WiFi connect info handler.
 * GET /wifiConnectInfo.json
 */
...
esp_err_t http_server_start(void)
{
  ...
  // Register AP SSID endpoint
  httpd_uri_t ap_ssid = {
      .uri = "/apSSID.json",
      .method = HTTP_GET,
      .handler = http_server_ap_ssid_handler,
      .user_ctx = NULL};
  err = httpd_register_uri_handler(s_server, &ap_ssid);
  if (err != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to register AP SSID URI: %s", esp_err_to_name(err));
    httpd_stop(s_server);
    s_server = NULL;
    return err;
  }

  // Register WiFi connect endpoint
  httpd_uri_t wifi_connect = {
      .uri = "/wifiConnect.json",
      .method = HTTP_POST,
      .handler = http_server_wifi_connect_handler,
      .user_ctx = NULL};
  err = httpd_register_uri_handler(s_server, &wifi_connect);
  if (err != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to register WiFi connect URI: %s", esp_err_to_name(err));
    httpd_stop(s_server);
    s_server = NULL;
    return err;
  }

  // Register WiFi connect status endpoint
  httpd_uri_t wifi_connect_status = {
      .uri = "/wifiConnectStatus",
      .method = HTTP_POST,
      .handler = http_server_wifi_connect_status_handler,
      .user_ctx = NULL};
  err = httpd_register_uri_handler(s_server, &wifi_connect_status);
  if (err != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to register WiFi connect status URI: %s", esp_err_to_name(err));
    httpd_stop(s_server);
    s_server = NULL;
    return err;
  }

  // Register WiFi connect info endpoint
  httpd_uri_t wifi_connect_info = {
      .uri = "/wifiConnectInfo.json",
      .method = HTTP_GET,
      .handler = http_server_wifi_connect_info_handler,
      .user_ctx = NULL};
  err = httpd_register_uri_handler(s_server, &wifi_connect_info);
  if (err != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to register WiFi connect info URI: %s", esp_err_to_name(err));
    httpd_stop(s_server);
    s_server = NULL;
    return err;
  }
