...
static esp_err_t ds3231_init_desc(i2c_port_t port, gpio_num_t sda_gpio, gpio_num_t scl_gpio)
{
    dev.port = port;
...
static void DS3231_task(void *pvParameter)
{
	// Initialize RTC
	if (ds3231_init_desc(I2C_NUM_0, CONFIG_SDA_GPIO, CONFIG_SCL_GPIO) != ESP_OK) {
	...
	for (;;)
	{
		...
		{
			sprintf(time_buffer, "%d.%d.%4d %02d:%02d:%02d",
					rtcinfo.tm_mday, rtcinfo.tm_mon+1, rtcinfo.tm_year, rtcinfo.tm_hour, rtcinfo.tm_min, rtcinfo.tm_sec);
